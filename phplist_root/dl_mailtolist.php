<?php
#This file is copied from PHPLIST (dl.php) and then modified.
# Search for Change_MAIL_TO_LIST-Begin to fine changes.
# Ideally dl_mailtolist.php must be placed in the same place as dl.php
# it is not needed to copy this file itself.
# if you just copy the changes to the file that comes with phplist that is enough.
# depending on what you do you will have to set the correct file path inside get_email.class.php point to the file with this code.
ob_start();
...
{
   $id = sql_escape(urldecode($_GET["fid"]));

   $data = Sql_Fetch_Row_Query("select filename,mimetype,remotefile,description,size from {$tables["attachment"]} where filename = \"$id\"");
}
#Change_MAIL_TO_LIST-End
